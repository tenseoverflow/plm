        ...
        <header className={`mb-6 ${className}`}>
            <div className="rounded-xl border border-calm-200/50 bg-gradient-to-r from-calm-400/15 via-calm-300/10 to-transparent p-5 dark:border-calm-800/50">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <div className="text-sm text-neutral-500">{new Date(dateStr).toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })}</div>
                        <h1 className="mt-1 text-3xl font-semibold tracking-tight">
                            {greeting}.
                        </h1>
                        <p className="mt-1 text-xl text-neutral-600 dark:text-neutral-400">{msg}</p>
                    </div>
                    {ui.showMindfulnessInWeek && (
                        <div className="flex flex-col items-stretch gap-2 sm:w-[380px]">
                            <div className="flex items-center justify-between gap-3">
                                ...
                                <MoodSelector value={mood} onChange={(v) => setMood(dateStr, v)} />
                            </div>
                            <Input
                                value={intention}
