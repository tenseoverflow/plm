...
export default function App() {
  ...
  const rollOverTasks = useAppState((s) => s.rollOverTasks);

  // Enable automatic sync when authenticated
  useAutoSync();

  // Automatic roll over unfinished tasks from yesterday to today
  useEffect(() => {
    const todayStr = todayString();
    const yesterdayStr = todayString(addDays(new Date(), -1));
    rollOverTasks(yesterdayStr, todayStr);
  }, [rollOverTasks]);
  ...
  return (
    <div className="min-h-screen">
      ...
      <footer className="px-4 pb-10 text-center text-neutral-400 text-xs">
...
function HeaderHome({ className }: { className?: string }) {
  ...
  return (
    <header className={`mb-6 ${className}`}>
      <div className="rounded-xl border border-calm-200/50 bg-linear-to-r from-calm-400/15 via-calm-300/10 to-transparent p-5 dark:border-calm-800/50">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-neutral-600 text-xl dark:text-neutral-400">
              {msg}
            </p>
          </div>
          {ui.showMindfulnessInWeek && (
            <div className="flex flex-col items-stretch gap-2 sm:w-[380px]">
              <div className="flex items-center justify-between gap-3">
                <span className="text-neutral-500 text-sm">How are you?</span>
                <MoodSelector
                  value={mood}
...
function SettingsLazy() {
  ...
  if (!Comp) return <div className="text-neutral-500 text-sm">Loading…</div>;
  return <Comp />;
}
