...
export default function Week() {
  ...
  function shiftWeek(delta: number) {
    const base = new Date(selectedWeekKey);
    const next = addDays(base, delta * 7);
    setSelectedWeekKey(startOfWeekString(next));
  }

  function onDrop(e: React.DragEvent, toDateStr: string) {
    e.preventDefault();
    e.stopPropagation();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-semibold text-xl">
            Week of {fmtDate(currentWeek.start)} - {fmtDate(currentWeek.end)}
          </h2>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => setSelectedWeekKey(startOfWeekString(new Date()))}
            className="rounded-md border border-neutral-300 px-3 py-1 text-sm dark:border-neutral-700"
          >
            This week
          </button>
      ...
      <div ref={scrollerRef} className="w-full overflow-x-auto">
        <div className="flex min-w-fit gap-4">
          {currentWeek.days.map(({ date, dateStr }, dayIndex) => {
            ...
            return (
              <Card
                key={dateStr}
                data-date={dateStr}
                className={`${isToday ? "border-calm-300 dark:border-calm-700" : "border-neutral-200 dark:border-neutral-800"} flex min-h-[560px] min-w-[360px] max-w-[420px] flex-col p-3 transition-colors`}
              >
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    ...
                    <div className="font-medium text-base">{fmtDate(date)}</div>
                  </div>
                  {isToday && (
                ...
                <ul
                  className="mt-2 min-h-[60px] space-y-1"
                  onDragOver={(e) => {
                    e.preventDefault();
                    ...
                    <button
                      ...
                      onClick={() => {
                        setWeeklyReportDraft(
                          weeklyReportsByWeekStart[selectedWeekKey] ?? "",
                        );
                        setShowWeeklyReportModal(true);
                      }}
                      className="w-full rounded-md bg-calm-600 px-3 py-2 text-sm text-white"
                    >
                      Weekly Report
                    </button>
                  </div>
      ...
      {showWeeklyReportModal && (
        <WeeklyReportModal
          ...
          onSave={(draft) => {
            saveWeeklyReport(selectedWeekKey, draft);
            setShowWeeklyReportModal(false);
          }}
          draft={weeklyReportDraft}
          setDraft={setWeeklyReportDraft}
          existingReport={weeklyReportsByWeekStart[selectedWeekKey]}
        />
      )}
    </div>
  );
  ...
  return (
    <Modal open={true} onClose={onMinimize} title={run.task.title}>
      <div className="space-y-3 text-center">
        ...
        <div className="font-bold text-5xl tabular-nums">{fmt(remaining)}</div>
...
function FocusMiniWidget({
  ...
  return (
    <div className="fixed right-4 bottom-4 z-40 rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm shadow-lg dark:border-neutral-700 dark:bg-neutral-900">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          {!running ? (
            <button
              onClick={onResume}
              className="rounded-md bg-calm-600 px-2 py-1 text-white text-xs"
            >
              Resume
            </button>
          ) : (
...
function AddInline({
  ...
  return (
    ...
    >
      ...
      />
      <button className="rounded-sm bg-calm-600 px-2 py-1 text-white text-xs">
        Add
      </button>
    </form>
  );
...
function WeeklyReportModal({
  ...
  useEffect(() => {
    if (existingReport) setDraft(existingReport);
  }, [existingReport, setDraft]);

  function save() {
    onSave(draft);
    if (isNew) {
      notify("Weekly Report", "Report saved");
    } else {
      notify("Weekly Report", "Report updated");
    }
  }

  return (
    <Modal open={true} onClose={onClose} title="Weekly Review">
      <div className="space-y-4 p-4 text-sm">
        <div className="text-neutral-500">
          {isNew
            ? "Write your reflections for the week. What went well, what didn't, and what you learned."
            : "Edit your weekly reflections."}
        </div>
        <textarea
          value={draft}
          onChange={(e) => setDraft(e.target.value)}
          className="w-full rounded-md border border-neutral-300 bg-white p-2 text-sm shadow-sm dark:border-neutral-700 dark:bg-neutral-900"
          rows={8}
        />
        <div className="flex justify-end gap-2">
          <button
