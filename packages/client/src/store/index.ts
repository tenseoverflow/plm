  ...
  persist(
    (set, _get) => ({
      ...
      rollOverTasks: (fromDate, toDate) =>
        set((s) => {
          const fromTasks = s.tasksByDate[fromDate] ?? [];
          const toTasks = s.tasksByDate[toDate] ?? [];
          const unfinished = fromTasks.filter((t) => !t.done);
          if (unfinished.length === 0) return s;
          return {
            tasksByDate: {
              ...s.tasksByDate,
              [fromDate]: fromTasks.filter((t) => t.done),
              [toDate]: [...toTasks, ...unfinished],
            },
          };
        }),
      ...
      logout: () => {
        // Clear all sensitive data from localStorage when logging out
        localStorage.removeItem("mindful-plm");

        return set(() => ({
          auth: {
            isAuthenticated: false,
          },
          // Reset all data to empty state for security
          moodByDate: {},
          intentionByDate: {},
          tasksByDate: {},
          journalByDate: {},
    ...
    }),
    {
      ...
      // Only persist to localStorage if user is NOT authenticated
      // When authenticated, data is stored on the server
      partialize: (s) => {
        if (s.auth.isAuthenticated) {
          // Only persist auth state and UI preferences when authenticated
          return {
            auth: s.auth,
            ui: s.ui,
          };
        }
        // Persist everything when not authenticated (local-only mode)
        return s;
      },
