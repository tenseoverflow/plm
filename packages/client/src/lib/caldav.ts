...
async function proxyFetch(
  input: RequestInfo,
  init?: RequestInit & { auth?: CalDavAuth },
) {
  ...
  if (typeof init?.body === "string") body = init.body as string;
    ...
    typeof (init as any).body.text === "function"
  ) {
...
export async function createOrUpdateTodo(
  ...
  const targetUrl =
    todo.href ||
    new URL(`${crypto.randomUUID()}.ics`, account.baseUrl).toString();
  ...
  if (!(res.status >= 200 && res.status < 300))
    throw new Error(`CalDAV save failed: ${res.status}`);
...
function parseMultiStatusForTodos(xml: string, baseUrl: string): RemoteTodo[] {
  ...
  for (const r of responses) {
    ...
    const calData =
      r.getElementsByTagName("calendar-data")[0]?.textContent || "";
    if (!calData) continue;
...
function parseIcsForVtodo(ics: string): {
  uid?: string;
  summary: string;
  completed: boolean;
  due?: string | null;
  ...
  for (const raw of lines) {
    ...
    if (line === "BEGIN:VTODO") inTodo = true;
      ...
      if (line.startsWith("SUMMARY:")) summary = line.slice(8).trim();
        ...
        completed = /COMPLETED/i.test(line.slice(7));
        ...
        if (/^\d{8}$/.test(v))
          due = `${v.slice(0, 4)}-${v.slice(4, 6)}-${v.slice(6, 8)}`;
        else if (/^\d{8}T/.test(v))
          due = `${v.slice(0, 4)}-${v.slice(4, 6)}-${v.slice(6, 8)}`;
      }
    }
  }
