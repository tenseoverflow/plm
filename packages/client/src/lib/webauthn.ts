...
export async function startPasskeyLogin(): Promise<{
  ...
  if (!res.ok) {
    const txt = await res.text().catch(() => "");
    console.error("Login start failed:", res.status, txt);
    throw new Error(`Failed to start login (${res.status}): ${txt}`);
  }

  const json = await res.json();
  console.log("Login start response:", json);
  const opts = json.options;

  // Convert base64url strings to ArrayBuffers
  opts.challenge = base64urlToBuffer(opts.challenge);
  if (Array.isArray(opts.allowCredentials)) {
    console.log("Converting allowCredentials:", opts.allowCredentials.length);
    opts.allowCredentials = opts.allowCredentials.map(
      (d: { id: string; type: string; transports?: string[] }) => ({
        ...d,
  ...
  console.log(
    "Login options prepared, credential count:",
...
export async function finishPasskeyLogin(
  ...
  console.log("Finishing passkey login...");
  const response = {
    id: bufferToBase64url(credential.rawId),
    rawId: bufferToBase64url(credential.rawId),
    type: credential.type,
  ...
  if (!res.ok) {
    const txt = await res.text().catch(() => "");
    console.error("Login finish failed:", res.status, txt);
